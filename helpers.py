...
def get_flags():
    ...
    tf.compat.v1.flags.DEFINE_integer('max_enc_seq_length', 50, 'Maximum encoder sequence length')
    tf.compat.v1.flags.DEFINE_integer('max_dec_seq_length', 50, 'Maximum decoder sequence length')
    tf.compat.v1.flags.DEFINE_integer('embedding_size', 32, 'Dimensionality of embedding')
    tf.compat.v1.flags.DEFINE_integer('num_units', 16, 'Number of units in the RNN cell')
    tf.compat.v1.flags.DEFINE_integer('batch_size', 64, 'Size of mini-batches during training')
    tf.compat.v1.flags.DEFINE_integer('epochs', 10, 'Number of epochs to train')
    tf.compat.v1.flags.DEFINE_integer('num_cells', 2, 'Number of RNN cells to use')
    tf.compat.v1.flags.DEFINE_integer('save_epochs', 10, 'Save model on every `save_epochs` epochs')
    tf.compat.v1.flags.DEFINE_integer('patience', 20, 'Allowed epochs without improving validation.')
    tf.compat.v1.flags.DEFINE_integer('random_seed', 42, 'Define random seed used for splitting data.')
    tf.compat.v1.flags.DEFINE_integer('max_features', 512, 'Number of word features.')
    tf.compat.v1.flags.DEFINE_integer('q_units', 32, 'Number of units in question state projections.')
    tf.compat.v1.flags.DEFINE_integer('update_every', 100, 'Update tensorboard summary on every  `update_every` training batch.')
    tf.compat.v1.flags.DEFINE_integer('beam_size', 1, 'Size of beam in beam search.')
    tf.compat.v1.flags.DEFINE_integer('parallel_iterations', 32, 'Number of parallel iterations used in the tf.while_loop.')
    tf.compat.v1.flags.DEFINE_float('learning_rate', 1e-3, 'Learning rate to use during training')
    tf.compat.v1.flags.DEFINE_float('max_gradient_norm', 5.0, 'Maximum allowed norm of gradients')
    tf.compat.v1.flags.DEFINE_float('keep_prob', 1.0, 'Input and output keep probability (inverse dropout)')
    tf.compat.v1.flags.DEFINE_float('alpha', 0.5, 'Trade-off parameter between learning sequences   and learning class predictions (1.0 is sequences only).')
    tf.compat.v1.flags.DEFINE_float('W_lambda', 0.0001, 'L2-regularization parameter.')
    tf.compat.v1.flags.DEFINE_float('mix_ratio', 0.50, 'Ratio of QA observations in the mixed batch loader.')
    tf.compat.v1.flags.DEFINE_string('model', 'RNN_LSTM', 'Choose model to train.')
    tf.compat.v1.flags.DEFINE_string('data', 'QA', 'Choose data to train on.')
    tf.compat.v1.flags.DEFINE_string('mode', 'train', 'Determine what mode to run the script in.                         Options are "train", "test", "plot".')
    tf.compat.v1.flags.DEFINE_string('sample_type', 'argmax', 'Determine what type of prediction sampling  is going to be used.')
    tf.compat.v1.flags.DEFINE_boolean('scheduled_sampling', False, 'Use scheduled sampling during traning.')
    tf.compat.v1.flags.DEFINE_boolean('clean', False, 'Reset saved model checkpoints.')
    tf.compat.v1.flags.DEFINE_boolean('verbose', True, 'Print information during training.')
    tf.compat.v1.flags.DEFINE_boolean('lm_support', False, 'Whether to use the corresponding                language model to support predictions.')
    tf.compat.v1.flags.DEFINE_boolean('swap_memory', False, 'Whether to use the `swap_memory` options     in the tf.while_loop.')
    tf.compat.v1.flags.DEFINE_boolean('create_summary', False, 'Add summary statistics for using         Tensorboard.')
    tf.compat.v1.flags.DEFINE_boolean('mix_data', False, 'Mix QA data with Wiki data.')

    return tf.compat.v1.flags.FLAGS
