...
class RNN:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Input placeholders
        self.encoder_input_chars = tf.compat.v1.placeholder(tf.int32, shape=[None, self.config.max_enc_seq_length],     name='encoder_input_chars')
        self.decoder_input_chars = tf.compat.v1.placeholder(tf.int32, shape=[None, self.config.max_dec_seq_length + 1], name='decoder_input_chars')
        self.target_chars        = tf.compat.v1.placeholder(tf.int32, shape=[None, self.config.max_dec_seq_length + 1], name='target_chars')

        self.enc_word_indices = tf.compat.v1.placeholder(tf.int32, shape=[None, self.config.max_words], name='enc_word_indices')

        self.encoder_sequence_length = tf.compat.v1.placeholder(tf.int32, shape=[None], name='encoder_sequence_length')
        self.decoder_sequence_length = tf.compat.v1.placeholder(tf.int32, shape=[None], name='decoder_sequence_length')
        self.word_seq_lengths        = tf.compat.v1.placeholder(tf.int32, shape=[None], name='encoder_word_sequence_length')

        # Categories
        self.class_idx = tf.compat.v1.placeholder(tf.int32, shape=[None], name='class_idx')

        # Placeholder for binary value telling the decoder whether to be in
        # "language model mode" or "question answering mode"
        self.is_lm_mode   = tf.compat.v1.placeholder(tf.int32, shape=[None], name='is_language_model_mode')

        # Placeholder for decoder output used for computing loss
        self.decoder_logits = tf.compat.v1.placeholder(tf.float32, shape=[None, self.config.max_dec_seq_length + 1, self.config.alphabet_size], name='decoder_logits')

        # Other placeholders
        self.global_step           = tf.Variable(0,   trainable=False, name='global_step')
        self.keep_prob_ph          = tf.Variable(1.0, trainable=False, name='input_keep_ph', dtype=tf.float32)
        self.probs_decay_parameter = tf.compat.v1.placeholder(tf.float64, shape=(), name='probs_decay_parameter')
        self.infer_token_prob      = tf.compat.v1.placeholder(tf.float64, shape=(), name='infer_token_prob')
